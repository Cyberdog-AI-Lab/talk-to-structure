...
ズーム・ドラッグ操作可能なインタラクティブな関係図として出力する Agent Skill です。情報の種類を見る「構造」モードと、AIが考えた架空の実データ例を1件ずつ追える「サンプルデータ」モードを切り替えられます。
...
AI：「何について整理したいですか？業務の話でもアイデアでも...」
         ↓（5フェーズのインタビュー）
AI：「output/index.html をブラウザで開いてください。構造とサンプルデータを切り替えられます」
```
...
[`output/index.html`](output/index.html) をブラウザで開くと、ライブハウス予約管理の例を確認できます。
...
> **表示確認には `output/index.html` を開いてください。**
> `.claude/skills/talk-to-structure/assets/graph-template.html` は生成元のテンプレートで、データが埋め込まれていないため直接開いても図は表示されません。
...
- **ズーム：** マウスホイール
- **移動：** ドラッグ
- **表示切替：** ヘッダーの「構造」／「サンプルデータ」
- **詳細確認：** ノード（四角）にホバー → 項目一覧・説明・ルールが表示
- **サンプル値の確認：** サンプルデータモードでノードにホバー → 実際の項目値が表示
- **つながりの強調：** サンプルノードをクリック → 直接つながるデータだけを強調（背景クリックで解除）
...
```json
{
  ...
  "sampleData": {
    "records": [
      { "id": "customer_1", "entity": "customer", "label": "青空商事",
        "values": { "顧客ID": "C001", "会社名": "青空商事" } },
      { "id": "order_1", "entity": "order", "label": "受注 O-001",
        "values": { "受注ID": "O-001", "顧客ID": "C001" } }
    ],
    "links": [
      { "id": "sl1", "relation": "r1", "from": "customer_1", "to": "order_1" }
    ]
  }
}
```

`sampleData` がない旧形式のJSONもそのまま表示できます。その場合、「サンプルデータ」ボタンは無効になります。
